...
Приложение для использования: Adobe Illustrator CS3-CS5
Версия: 1.1
...
Application to use with: Adobe Illustrator CS3-CS5
Version: 1.1
	...
	if ( CHECK_SELECTION()) 
	{
		...
		function HOR_CB_GROUP( x, y, where )
		{
			...
			for( var i = 0; i < 3; i++ )
			{
				var dx = 0;
				if( i == 0 ) { if(WFS)  dx = -3 };
				if( i == 1 ) { dx = -5; if(WFS)  dx = -6 };
				if( i == 2 ) { dx = -9; if(WFS)  dx = -10 };
		...
		function VER_CB_GROUP( x, y, where )
		{
			...
			{
				var dy = 0;
				if( i == 0) { dy = 0; if(WFS)  dy = 0 };
				if( i == 1) { dy = -2; if(WFS)  dy = -3 };
				if( i == 2) { dy = -5; if(WFS)  dy = -5 };
		...
		function MAKE_DIRECTIONS(x, y)
		{
			...
			//
			var button_dx = 0;  if(WFS)  button_dx = 2 ;
			b_left = b_left - button_dx
			// создаем кнопку снять все
			var UNCH_TEXT = "Uncheck all"; if( !WFS ) UNCH_TEXT = "Uncheck";
		...
		var make_dir_dy = 2; if(WFS)  make_dir_dy = 0 ;
		...
		function digit_on_Changing(et, def, emp) 
		{
			...
			for( var i=0; i < s.length; i++ ) 
			{
				...
				if
				( 
					((si != "-" && si != "+" && si != "." && si != "," && NO_DIGIT_SYMBOL(si)) || N_C>1 ) ||
					((N_P + N_M)>1) ||
					( si == "-"  && i !=0 ) ||
					( si == "+"  && i !=0 )
				) 
		...
		function MAKE_OPTIONS_PANEL(x, y)
		{
			...
			un_drop = MAKE_DROP( mp, un_drop_left, pan_y_units, un_drop_w, un_drop_h, ["mm", "cm", "pt", "in"] , un_drop_ini );
			...
			function sw_on_Changing()
			{
				...
				if( BAD_SW ) { BAD_SW = false; str_w_ed_text.text = SW_NUM_INI; return};
				if( t == "-" )
				{
					str_w_ed_text.text = SW_NUM_INI;
					BAD_SW = true;
			...
			str_un_drop = MAKE_DROP( mp, str_un_drop_left, str_w_y, str_un_drop_w, str_un_drop_h, ["mm", "cm", "pt", "in"] , SW_UN_INI);
			...
			g_m.add('item', "Nothing");
			g_m.add('item', "Marks only");
			if( N_sel > 0 ) g_m.add('item', "Marks and selection");
		...
		{
			// только один объект
			if( N_sel == 1 ) OBJ_DROP.add('item', "Selected object");
			// если больше одного объекта
			if( N_sel >= 2 ) 
			{
				OBJ_DROP.add('item', "Entire selection");
				OBJ_DROP.add('item', "Each object in selection");
			};// if
		};// if
		...
		if (DIALOG_BUTTON == 1) 
		{
			...
			{
				alert("Bad value input!\n\nCrop mark length can not be < or = zero");
				BAD_VAL = true;
				return;
			};// if
			// отступ (офсет)
...
function ADDING() 
{
	...
	if( OBJECTS_TO_MAKE == "Entire selection" || 
		 OBJECTS_TO_MAKE == "Each object in selection" || 
		 OBJECTS_TO_MAKE == "Selected object")
	{
	...
	if( OBJECTS_TO_MAKE == "Entire selection" || OBJECTS_TO_MAKE == "Selected object")
	{
	...
	if( OBJECTS_TO_MAKE == "Each object in selection" )
	{
		for( var i = 0; i < N_sel; i++ ) MARKS( left_arr[i], top_arr[i], bottom_arr[i], right_arr[i] );
	};// if Each object in selection
	var g_err = false;
	// если задана группировка меток
	if( g_m == "Marks only" || g_m == "Marks and selection")
	{
		// группируем метки
		try
		{
			if( ALL_MARKS.length > 1 ) 
			{
				...
				for( var i=0; i < ALL_MARKS.length; i++ ) ALL_MARKS[i].move(G_M, ElementPlacement.INSIDE);
			}
	...
	if( g_m == "Marks and selection")
	{
		...
		try
		{
			...
			else 
			{
				...
				for( var i=0; i < N_sel; i++ ) the_sel[i].move(G_S, ElementPlacement.INSIDE);
			};// if-else
		} catch ( error ) {  g_err = true };
...
function CHECK_SELECTION() 
{
	...
	{
		 alert("There are some selected guides!"+BL+"Can not process that!");
...
function INI_WRITE()
{
	...
	SW_NUM_INI = TEXT_TO_DIGIT(str_w_ed_text.text)+""; SW_UN_INI = SW_UN_INI+""; 
	if( w_cont ) W_CONT_INI ="1" else W_CONT_INI = "0";
	g_m_ini = g_m_ini +"";
	if( BOUNDS_TO_MAKE == "Visible" ) BOUNDS_INI = "1" else BOUNDS_INI = "0";
...
function FILE_IO( n )
{
	...
	try 
	{
		if( n ) 
		{
			w[0] = LENGTH_VER_INI; w[1] = LENGTH_HOR_INI; 
			w[2] = OFFSET_VER_INI; w[3] = OFFSET_HOR_INI;
			w[4] = BLEED_VER_INI;    w[5] = BLEED_HOR_INI;
			w[6] = SW_NUM_INI;       w[7] = SW_UN_INI; 
			w[8] = W_CONT_INI;       w[9] = BOUNDS_INI;
			w[10] = g_m_ini;
		};// if
		...
		if( !n ) 
		{
			LENGTH_VER_INI = r[0]; LENGTH_HOR_INI =  r[1]; 
			OFFSET_VER_INI = r[2]; OFFSET_HOR_INI = r[3];
			BLEED_VER_INI = 	 r[4];  BLEED_HOR_INI =   r[5];
			SW_NUM_INI =      r[6]; SW_UN_INI =          r[7]; 
			W_CONT_INI =      r[8]; BOUNDS_INI =         r[9];
			g_m_ini = r[10];
	...
	try{ INI_FILE.close() } catch( error ) {};
	return;
};// end FILE_IO
//
...
function ALREADY_MADE(L, T, R, B, C)
{
	...
	try
	{
		...
		{
			if( L.toFixed(d) == L_MADE[i].toFixed(d) && 
				 T.toFixed(d) == T_MADE[i].toFixed(d) && 
				 R.toFixed(d) == R_MADE[i].toFixed(d) && 
				 B.toFixed(d) == B_MADE[i].toFixed(d) && C == C_MADE[i] ) return true;
		};// for i
	} catch ( error ) {};
	return false;
};// end ALREADY_MADE