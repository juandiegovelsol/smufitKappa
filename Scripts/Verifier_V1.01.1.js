...
dlg.simbolos.sk = dlg.simbolos.add('checkbox', [10,20,180,40], 'Smurfit Kappa');
dlg.simbolos.escuadra = dlg.simbolos.add('checkbox', [10,50,180,70], 'Escuadra de registro');
dlg.simbolos.cajetin = dlg.simbolos.add('checkbox', [10,80,180,100], 'Ficha');
...
function anadirCosas(){
    ...
    ***************************************************************************************************************************/ 
    ...
    var fichero =  'C:\\Program\ Files\\Adobe\\Adobe\ Illustrator\ 2023\\Presets\\es_ES\\Símbolos\\SmurfitKappa\ -\ Ficha.ai';
...
//v1.01 se modifica el condicional para que no cree la capa si no seleccionamos que nos pegue los simbolos.
    if (valorSK == true || valorEscuadra == true){
        ...
        if(valorSK == true){        
            try {
                libSKFicha.duplicar(doc,logoSK2).position = [posOriX + 40, posOriY + 40];
            }catch(err){
        ...
        if(valorEscuadra == true){
            try {
                ...
                libSKFicha.duplicar(doc,escuadra).position = [posOriX + 70, posOriY + 70];
            }catch(err){
...
function ARDcrearCapaSangrado(mm) {
	...
	capaARD2.name = activeDocument.name.substr(0, activeDocument.name.indexOf("."));
	app.paste();
...
function buscarCapaARD() {
	...
	for (var i=0; i<doc.layers.length; i++)
		if ( doc.layers[i].name.indexOf(".ARD") >= 0 )
				return doc.layers[i];	
...
function symbolLibrary() {
    ...
    this.duplicar = function (targetDoc, nombre) {
        var simboloItem = this.symbolsDoc.symbolItems.add(this.symbolsDoc.symbols.getByName(nombre));
    ...
    this.simbolo = function (nombre) {
        var simbolItem = this.symbolsDoc.symbolItems.add(this.symbolsDoc.symbols.getByName(nombre));
        return simbolItem;
    };
